...
$get_info = mysqli_query($link,"SELECT company, position, school, concentration FROM users WHERE username='$user'");
...
if ($updatework) {
	...
	$position = ucwords($position);
		...
		$info_submit_query = mysqli_query($link,"UPDATE users SET company='$company', position='$position' WHERE username='$user'");
...
if ($updateinfo) {
	...
	$concentration = ucwords($concentration);
		...
		//submit the form to database
		$info_submit_query = mysqli_query($link,"UPDATE users SET school='$school', concentration='$concentration' WHERE username='$user'");
		echo "<p class='error_echo'>Your Profile Information Has Been Updated.</p>";
		header("Location: about.php?u=$user");
}
