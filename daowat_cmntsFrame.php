...
if (isset($_POST['daowatComment' . $getid . ''])) {
	...
	$query = mysqli_query($link,"SELECT id,added_by  FROM daowat WHERE id='$getid'");
	$query_row = mysqli_fetch_assoc($query);
	$daowat_to = $query_row['added_by'];
	$insertPost = mysqli_query($link,"INSERT INTO daowat_comments VALUES ('','$daowat_body','$date_added',NOW(),'$user','$daowat_to','no','$getid')");
}
}
//post query
	$query = mysqli_query($link,"SELECT id,added_by  FROM daowat WHERE id='$getid'");
	...
	$dwtby_query = mysqli_query($link,"SELECT * FROM users WHERE username='$daowat_to'");
	$dwtby_gender_row = mysqli_fetch_assoc($dwtby_query);
	$dwtby_gender_value = $dwtby_gender_row['gender'];
	//getting user gender
	$usergender_query = mysqli_query($link,"SELECT * FROM users WHERE username='$user'");
...
<?php 
	if ($dwtby_gender_value == 2) {
		...
		$get_msg_num = mysqli_query($link,"SELECT * FROM pvt_messages WHERE user_from='$daowat_to' AND user_to='$user' LIMIT 2");
...
<?php
//Get relevant daowat
$get_comments = mysqli_query($link,"SELECT * FROM daowat_comments WHERE daowat_id='$getid' ORDER BY id DESC");
...
while ($comment = mysqli_fetch_assoc($get_comments)) {
	...
	$get_user_info = mysqli_query($link,"SELECT * FROM users WHERE username='$dawat_by'");
	$get_info = mysqli_fetch_assoc($get_user_info);
	$profile_pic_db= $get_info['profile_pic'];
	$daowat_by = $get_info['first_name'];
	$dwt_user_info = mysqli_query($link,"SELECT * FROM users WHERE username='$daowat_by'");
	$fname_info = mysqli_fetch_assoc($dwt_user_info);
	
	//check for propic delete
						$pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$dawat_by' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
