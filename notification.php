...
//name query
$about_query = mysqli_query($link,"SELECT first_name FROM users WHERE username='$user'");
	...
	<?php include ( "./inc/header.inc.php" ); ?>
				 		...
				 		<ul>
				 			<li style='line-height: 8px;'>
				 				<?php 
									...
									$get_notipost = mysqli_query($link,"SELECT * FROM post_comments WHERE posted_to='$user' AND posted_by != '$user' ORDER BY id DESC LIMIT 50");
									$count_notipost = mysqli_num_rows($get_notipost);
				 					//getting daowat comment
									$get_notidwt = mysqli_query($link,"SELECT * FROM daowat_comments WHERE daowat_to='$user' AND daowat_by != '$user' ORDER BY id DESC LIMIT 50");
									$count_notidwt = mysqli_num_rows($get_notidwt);
									$count = $count_notidwt + $get_notidwt;
									if ($count != 0) {
										//getting post noti
										while ($noti = mysqli_fetch_assoc($get_notipost)) {
		...
		if ($pro_changed_db != $profile_pic_db) {
			...
			$profile_pic = "userdata/profile_pics/".$profile_pic_db;
											...
											$get_descrip = mysqli_query($link,"SELECT * FROM posts WHERE id='$post_id'");
