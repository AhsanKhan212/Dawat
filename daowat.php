...
$num = mysqli_num_rows($result);
	if ($num == 1) {
		...
		$daowat =  trim($daowat);
		$daowat = mysqli_real_escape_string($link,$daowat);
		...
		?>
			...
			<?php echo $error ?>
				<div style="width: 560px; margin: 15px auto;">
					...
					<?php
					//for getting post
					$getposts = mysqli_query($link,"SELECT * FROM posts WHERE added_by ='$username' AND (daowat_body !='' || daowat_post != '0') ORDER BY id DESC LIMIT 5 ") or die(mysqli_error());
