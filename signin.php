...
if (isset($_POST['login'])) {
		if (isset($_POST['user_login']) && isset($_POST['password_login'])) {
			...
			$password_login = mysqli_real_escape_string($link,$_POST['password_login']);
			$rememberme = $_POST['rememberme'];		
			$num = 0;
			$password_login_md5 = md5($password_login);
			$result = mysqli_query($link,"SELECT * FROM users WHERE (username='$user_login' || email='$user_login') AND password='$password_login_md5' AND activated='1' AND blocked_user='0'");
			...
			if ($num>0) {
				...
				$result1 = mysqli_query($link,"SELECT * FROM users WHERE (username='$user_login' || email='$user_login') AND password='$password_login_md5' AND activated ='0'");
				...
				if ($num1>0) {
					...
					$result1 = mysqli_query($link,"SELECT * FROM users WHERE (username='$user_login' || email='$user_login') AND password='$password_login_md5' AND blocked_user='1'");
					$num1 = mysqli_num_rows($result1);
					if ($num1>=1) {
						$success_message = '
...
if (isset($_POST['submconfrmCode'])) {
	...
	$result2 = mysqli_query($link,"SELECT * FROM users WHERE username='$user_loginnn' AND confirmCode='$user_confrmCode_db' AND activated='0'");
...
if(isset($_POST["name2check"]) && $_POST["name2check"] != ""){
    ...
    $sql_uname_check = mysqli_query($link,"SELECT id FROM users WHERE username='$username' LIMIT 1"); 
...
$_POST['username'] = preg_replace('/\s+/','',$_POST['username']);
	try {
		...
		//username check
		$u_check = mysqli_query($link,"SELECT username FROM users WHERE username='$u_name'");
		$check = mysqli_num_rows($u_check);
		// Check if email already exists
		$e_check = mysqli_query($link,"SELECT email FROM users WHERE email='$u_email'");
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >4 ) {
						...
						$result = mysqli_query($link,"INSERT INTO users (first_name,username,email,password,gender,sign_up_date,activated) VALUES ('$_POST[first_name]','$_POST[username]','$_POST[email]','$_POST[password]','$_POST[gender]','$d','1')");
...
$getposts = mysqli_query($link,"SELECT * FROM daowat WHERE photos != '' ORDER BY  RAND() ");
