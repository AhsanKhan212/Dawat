...
$get_info = mysqli_query($link,"SELECT mobile,pub_email,email FROM users WHERE username='$user'");
...
if ($sendemail) {
	...
	$pub_email = mysqli_real_escape_string($link,$pub_email);
		//submit the form to database
		$info_submit_query = mysqli_query($link,"UPDATE users SET mobile='$mobile' WHERE username='$user'");
		$info_submit_query = mysqli_query($link,"UPDATE users SET pub_email='$pub_email' WHERE username='$user'");
...
if ($emaildata) {
	...
	$check_for_email = mysqli_query($link,"SELECT * FROM users WHERE email='$email'");
	...
	if ($email == "") {
		...
		//getting email
		$check_for_email1 = mysqli_query($link,"SELECT * FROM users WHERE username='$user'");
		...
		$info_submit_query = mysqli_query($link,"UPDATE users SET email='$email', confirmCode='$confirmCode' WHERE username='$user'");
		echo "<script>window.open('cbinfo_update.php','_self')</script>";
