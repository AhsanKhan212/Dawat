...
//check for userfrom propic delete
						$pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$user' AND (discription='updated his cover photo.' OR discription='updated her cover photo.') ORDER BY id DESC LIMIT 1");
...
$check_pic = mysqli_query($link,"SELECT profile_pic FROM users WHERE username='$user'");
$get_pic_row = mysqli_fetch_assoc($check_pic);
$profile_pic_db = $get_pic_row['profile_pic'];
//check for userfrom propic delete
						$pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$user' AND (discription='changed his profile picture.' OR discription='changed her profile picture.' ORDER BY id DESC LIMIT 1");
						//$get_pro_changed = mysqli_fetch_assoc($pro_changed);
						if ($pro_changed){
						   $get_pro_changed = mysqli_fetch_assoc($pro_changed);
...
<table>
	<tbody>
		...
		$getposts = mysqli_query($link,"SELECT * FROM posts WHERE newsfeedshow ='1' AND report ='0' AND note='0' AND daowat_give='0' ORDER BY id DESC") or die(mysqli_error());
		...
		while ($row = mysqli_fetch_assoc($getposts)) {
				...
				if ($added_by == $user) {
					...
					$checkDeactiveUser= mysqli_query($link,"SELECT * FROM users WHERE username = '$added_by'") or die(mysqli_error());
					$checkDeactiveUser_row = mysqli_fetch_assoc($checkDeactiveUser);
					$activeOrNot = $checkDeactiveUser_row ['activated'];
					if ($activeOrNot != '0') {					
						$check_if_follow = mysqli_query($link,"SELECT * FROM follow WHERE (user_from='$user' AND user_to='$added_by ') ORDER BY id DESC LIMIT 2");
