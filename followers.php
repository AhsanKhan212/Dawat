			...
			$count_frnd_num = 0;
				...
				echo '<div>
					...
					while ($row=mysqli_fetch_array($queryfollowers )) {
						...
						$user_to = $row['user_to'];
						//spacif user profile
						$family_query = mysqli_query($link,"SELECT id,username,first_name,profile_pic,cover_pic,city,hometown,company,school,gender,verify_id FROM users where username='$user_from' ORDER BY id DESC LIMIT 1");
...
$check_pic = mysqli_query($link,"SELECT cover_pic FROM users WHERE username='$user'");
$get_pic_row = mysqli_fetch_assoc($check_pic);
$cover_pic_db = $get_pic_row['cover_pic'];
//check for userfrom propic delete
						$pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$user_from' AND (discription='updated his cover photo.' OR discription='updated her cover photo.') ORDER BY id DESC LIMIT 1");
						$get_pro_changed = mysqli_fetch_assoc($pro_changed);
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
