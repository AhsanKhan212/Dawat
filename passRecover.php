...
$check_pic = mysqli_query($link,"SELECT profile_pic, first_name, username FROM users WHERE username='$username' || email='$username'");
$get_pic_row = mysqli_fetch_assoc($check_pic);
$profile_pic_db = $get_pic_row['profile_pic'];
$pic_uname = $get_pic_row['username'];
$fullname_db = $get_pic_row['first_name'];
//check for propic delete
$pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$pic_uname' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
...
if (isset($_POST['searchId'])) {
	...
	$username_query = mysqli_query($link,"SELECT * FROM users WHERE username='$username' || email='$username'");
...
$recovEmail = mysqli_real_escape_string($link,$recovEmail);
if (isset($_POST['searchRecovEmail'])) {
	$username = $_SESSION['username'];
	$searchRecovEmail_query = mysqli_query($link,"SELECT * FROM users WHERE (email='$recovEmail' AND username='$username')");
	...
	if ($searchRecovEmail_query_num >= 1) {
		...
		if (mysqli_query($link,$confirmCodeQuery)) {
...
if (isset($_POST['searchRecovEmail2'])) {
	...
	$searchRecovEmail_query = mysqli_query($link,"SELECT * FROM users WHERE (email='$user_con_email' AND username='$user_con_name')");
	$searchRecovEmail_query_num = mysqli_num_rows($searchRecovEmail_query);
	$usernm_fetch_query = mysqli_fetch_assoc($searchRecovEmail_query);
	$get_first_nm_fetch_query = $usernm_fetch_query['first_name'];
	if ($searchRecovEmail_query_num >= 1) {
...
$confirmMailCode = mysqli_real_escape_string($link,$confirmMailCode);
$newpassword = mysqli_real_escape_string($link,$newpassword);
$repear_password = mysqli_real_escape_string($link,$repear_password);
if (isset($_POST['confrmRessetpass'])) {
	$unameCnfrm = $_SESSION['final_uname'];
	if ($unameCnfrm != '') {
		$unameCnfrm_query = mysqli_query($link,"SELECT * FROM users WHERE username='$unameCnfrm'");
		...
		if ($unameCnfrm_query_num >= 1) {
			...
			if ($confirmMailCode == $get_confirmCode_fetch_query) {
				if ($newpassword == $repear_password) {
					...
					if (mysqli_query($link,$updatePassQuery)) {
