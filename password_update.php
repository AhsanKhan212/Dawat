...
if ($senddata) {
	...
	$password_query = mysqli_query($link,"SELECT * FROM users WHERE username='$user'");
	while ($row = mysqli_fetch_assoc($password_query)) {
		...
		if ($oldpassword_md5 == $db_password) {
			if ($newpassword == $repear_password) {
				...
				$password_update_query = mysqli_query($link,"UPDATE users SET password='$newpassword_md5', confirmCode='$confirmCode' WHERE username='$user'");
