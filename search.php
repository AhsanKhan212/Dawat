	...
	<?php
		if (($_GET['keywords'] && $_GET['topic']) == NULL) {
			...
			if (isset($_GET['keywords'])) {
				...
				$query = mysqli_query($link,$query) or die ("could not count");
			...
			if ($count == 0){
				...
				echo '<div class="search_banner">Result for: 
					...
					while ($row=mysqli_fetch_array($query)) {
						...
						$pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$username' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
		...
		if ($pro_changed_db != $profile_pic_db) {
			...
			$profile_pic = "userdata/profile_pics/".$profile_pic_db;
						...
						$pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$username' AND (discription='updated his cover photo.' OR discription='updated her cover photo.') ORDER BY id DESC LIMIT 1");
						$get_pro_changed = mysqli_fetch_assoc($pro_changed);
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
