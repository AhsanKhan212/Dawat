...
$get_info = mysqli_query($link,"SELECT bio,queote FROM users WHERE username='$user'");
...
if ($updateinfo) {
	...
	$bio = mysqli_real_escape_string($bio);
		...
		$info_submit_query = mysqli_query($link,"UPDATE users SET bio='$bio' WHERE username='$user'");
...
if ($update) {
	...
	$queote = mysqli_real_escape_string($queote);
		//submit the form to database
		$info_submit_query = mysqli_query($link,"UPDATE users SET queote='$queote' WHERE username='$user'");
