...
<?php 
	...
	$get_title_info = mysqli_query($link,"SELECT * FROM users WHERE username='$username'");
...
<body>
	...
	if (($msg_count >=1 ) || ($username == $user)) {
			...
			$count = "";
				...
				$queryfollowing= mysqli_query($link,$queryfollowing) or die ("could not count");
				$countfollowing = mysqli_num_rows($queryfollowing);
				//count followers
				$queryfollowers = "SELECT * FROM follow WHERE user_to='$username' ORDER BY id DESC";
				$queryfollowers = mysqli_query($link,$queryfollowers ) or die ("could not count");
				...
				$queryfollowing2= mysqli_query($link,$queryfollowing2) or die ("could not count");
				while ($row=mysqli_fetch_array($queryfollowing2)) {
						$user_from = $row['user_from'];
						$user_to = $row['user_to'];
						//user to if friend 
						
						$if_user_to_follow = mysqli_query($link,"SELECT * FROM follow WHERE (user_from='$user_to' AND user_to='$user_from')");
				...
				echo '<div>
					...
					if ($count_frnd_num == 0){
						...
						';
					while ($row=mysqli_fetch_array($queryfollowing)) {
						$user_from = $row['user_from'];
						$user_to = $row['user_to'];
						//user to if friend 
						...
						//spacif user profile
						$family_query = mysqli_query($link,"SELECT id,username,first_name,profile_pic,cover_pic,city,hometown,company,school,gender,verify_id FROM users where username='$user_to' ORDER BY id DESC LIMIT 1");
		...
		if ($pro_changed_db != $profile_picuser_to_db) {
			...
			$profile_picuser_to = "userdata/profile_pics/".$profile_picuser_to_db;
						...
						$pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$user_to' AND (discription='updated his cover photo.' OR discription='updated her cover photo.') ORDER BY id DESC LIMIT 1");
						$get_pro_changed = mysqli_fetch_assoc($pro_changed);
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
