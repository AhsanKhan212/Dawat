...
if (isset($_REQUEST['pid'])) {
	...
	$id = mysqli_real_escape_string($link,$_REQUEST['pid']);
}else {
...
//for getting post
$getposts = mysqli_query($link,"SELECT * FROM posts WHERE id ='$id'") or die(mysqli_error());
...
$get_user_info = mysqli_query($link,"SELECT * FROM users WHERE username='$added_by'");
...
$get_fname_info = mysqli_query($link,"SELECT * FROM users WHERE username='$user_posted_to'");
...
$get_sharepost = mysqli_query($link,"SELECT * FROM posts WHERE id='$sharepostid'");
...
$sp_get_user_info = mysqli_query($link,"SELECT * FROM users WHERE username='$sp_added_by'");
...
$sp_add_by = $sp_get_info['first_name'];
		...
		//check for propic delete
					$sp_pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$sp_added_by' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
	...
	//check for propic delete
					$pro_changed = mysqli_query($link,"SELECT * FROM posts WHERE added_by='$added_by' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
...
$get_comments = mysqli_query($link,"SELECT * FROM post_comments WHERE post_id='$id' ORDER BY id DESC LIMIT 3");
...
<?php
//count comment
$get_comments = mysqli_query($link,"SELECT * FROM post_comments WHERE post_id='$id' ORDER BY id DESC ");
$count = mysqli_num_rows($get_comments);
//getting all like
$get_like = mysqli_query($link,"SELECT * FROM post_likes WHERE post_id='$id' ORDER BY id DESC");
...
//showing data on profile
		...
		echo "<div style='min-height: 55px;' ><div style='float: left;'><img src='$profile_pic' style= 'border-radius: 22px'; title=\"$added_by\" height='45' width='45'  /></div>";
			...
			if($sharepostid != 0) {
				...
				$sp_photos = "http://www.daowat.com/userdata/profile_pics/".$sp_photos_db;
					...
					<div style="padding: 10px;" >';
						...
						echo'<div>
							<a href="profile.php?u='.$sp_added_by.'" class="" style="float: left;" ><img src="'.$sp_profile_pic.'" style="height: 33px; width: 33px; border: 1px solid #ddd; border-radius: 5px;"  /></a>
						</div>
			...
			$like_query = mysqli_query($link,"SELECT * FROM post_likes WHERE user_name='$user' AND post_id='$id' ORDER BY id DESC LIMIT 1");
